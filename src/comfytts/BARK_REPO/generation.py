...
CACHE_DIR = "D:/ComfyUI/models/comfytts/BarkModel"
...
def _load_history_prompt(history_prompt_path):
    if isinstance(history_prompt_path, str) and history_prompt_path.endswith(".npz"):
        history_prompt = np.load(history_prompt_path)
    elif isinstance(history_prompt_path, str):
        # make sure this works on non-ubuntu
        history_prompt_input = os.path.join(*history_prompt_path.split("/"))
        if history_prompt_input not in ALLOWED_PROMPTS:
            raise ValueError("history prompt not found")
        history_prompt = np.load(
            os.path.join(CUR_PATH, "assets", "prompts", f"{history_prompt_input}.npz")
        )
    elif isinstance(history_prompt_path, dict):
        assert("semantic_prompt" in history_prompt_path)
        assert("coarse_prompt" in history_prompt_path)
        assert("fine_prompt" in history_prompt_path)
...
def generate_text_semantic(
    ...
    if history_prompt_path is not None:
        history_prompt = _load_history_prompt(history_prompt_path)
